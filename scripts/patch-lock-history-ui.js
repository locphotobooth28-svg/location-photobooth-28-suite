...
const lockEnd=`<div className="bp-status-ok" style={current.lockScreen?.locked?{}:{color:'#cbd5e1',background:'rgba(148,163,184,.08)',borderColor:'var(--line)'}}>{current.lockScreen?.locked?'🔒 Borne actuellement verrouillée':current.lockScreen?.enabled?'🕐 Verrouillage programmé':'⚪ Verrouillage désactivé'}</div></div>}`;
if(!s.includes(lockEnd))throw new Error('[lock-history-ui] fin onglet verrouillage introuvable');
const lockEndNew=`<div className="bp-status-ok" style={current.lockScreen?.locked?{}:{color:'#cbd5e1',background:'rgba(148,163,184,.08)',borderColor:'var(--line)'}}>{current.lockScreen?.locked?'🔒 Borne actuellement verrouillée':current.lockScreen?.enabled?'🕐 Verrouillage programmé':'⚪ Verrouillage désactivé'}</div><div className="bp-card" style={{marginTop:14}}><div style={{display:'flex',justifyContent:'space-between',gap:10,alignItems:'center',flexWrap:'wrap'}}><div><h4 style={{margin:0}}>🕒 Historique de verrouillage</h4><div className="muted" style={{marginTop:5}}>Programmations, verrouillages, déverrouillages et synchronisation du PIN. Le code PIN n'est jamais affiché.</div></div><button className="ghost" onClick={()=>loadLockHistory(current.boothName)}>↻ Actualiser</button></div>{lockHistoryLoading?<div className="muted" style={{marginTop:14}}>Chargement…</div>:lockHistoryError?<div className="notice error" style={{marginTop:14}}>{lockHistoryError}</div>:lockHistory.length===0?<div className="muted" style={{marginTop:14}}>Aucun historique de verrouillage.</div>:<div style={{marginTop:12}}>{lockHistory.map(h=><div key={h.id||h.at} className="bp-row" style={{alignItems:'flex-start'}}><div><b>{lockHistoryLabel(h)}</b>{lockHistoryDetails(h)&&<div className="muted" style={{fontSize:12,marginTop:3}}>{lockHistoryDetails(h)}</div>}</div><span className="muted" style={{whiteSpace:'nowrap'}}>{h.at?new Date(h.at).toLocaleString('fr-FR'):'—'}</span></div>)}</div>}</div></div>}`;
s=s.replace(lockEnd,lockEndNew);
