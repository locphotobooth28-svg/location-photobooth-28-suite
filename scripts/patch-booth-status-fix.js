...
if (source.includes(oldCode)) { source = source.replace(oldCode, newCode); changes++; }

if (source.includes('giftAmount:0')) { source = source.replace('giftAmount:0','giftAmount:sumTotal(giftedEvents)'); changes++; }
...
if(source.includes(oldSummary)){ source=source.replace(oldSummary,newSummary); changes++; }
...
if(source.includes(oldPeriod)){ source=source.replace(oldPeriod,newPeriod); changes++; }
...
if(!source.includes('const [historyBooth,setHistoryBooth]')){
  ...
  source=source.replace(stateOld,stateNew); changes++;
}
...
if(!source.includes('async function openHistory(name)')){
  ...
  source=source.replace(agoLine,helpers); changes++;
}
...
if(!source.includes('>🧾 Historique imprimante</button>')){
  ...
  source=source.replace(commOld,commNew); changes++;
}
...
if(!source.includes('{historyBooth&&<div className="modal-backdrop"')){
  ...
  source=source.slice(0,fnStart)+patched+source.slice(fnEnd); changes++;
}

if(source.includes('Version 8.5.87')){ source=source.split('Version 8.5.87').join('Version 8.5.88'); changes++; }
