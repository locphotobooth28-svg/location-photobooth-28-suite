...
// Stable compatibility patch. The source may already contain the lock feature
// from a previous build/patch, so never fail only because an older anchor moved.
...
if(!s.includes('lockScreen:{enabled:Boolean(p?.lockScreen?.enabled)')){
  ...
  if(idx>=0){s=s.slice(0,idx)+lockRead+s.slice(idx);changes++;}
}
...
if(!s.includes('lockScreen:{enabled:controls[name].lockScreen.enabled')){
  ...
  if(s.includes(configuredOld)){s=s.replace(configuredOld,configuredNew);changes++;}
}
...
if(!a.includes('async function saveLockSchedule(boothName)')){
  ...
  if(a.includes(helperAnchor)){a=a.replace(helperAnchor,helper+helperAnchor);changes++;}
}
...
if(!a.includes('lp28-lock-schedule-admin')){
  ...
  if(a.includes(uiAnchor)){a=a.replace(uiAnchor,ui+uiAnchor);changes++;}
}
fs.writeFileSync(appPath,a,'utf8');
console.log(`[LP28] patch-booth-lock-schedule stable : ${changes} modification(s) appliquée(s).`);
