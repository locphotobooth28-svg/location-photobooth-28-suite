...
function ensureStyles(){
  ...
  style.textContent=`
    ...
    .lp28-doc-count-badge,.lp28-doc-new-badge{
      display:inline-flex;align-items:center;justify-content:center;min-width:24px;height:24px;padding:0 7px;
      border-radius:999px;font-size:12px;font-weight:900;line-height:1;white-space:nowrap;
    }
    ...
    .lp28-payment-status{
      display:inline-flex;align-items:center;gap:6px;margin:4px 0 8px;padding:5px 9px;border-radius:999px;
      font-size:12px;font-weight:800;border:1px solid rgba(217,182,75,.45);background:rgba(217,182,75,.08);color:#f2d16d;
    }
...
function findDocumentsNav(){
  ...
  return nodes.find(el=>{
    ...
    return /^📄?\s*Documents(?:\s*\d+)?$/i.test(t) || /^Documents(?:\s*\d+)?$/i.test(t);
  }) || nodes.find(el=>normalizeText(el.textContent).toLowerCase()==="documents") || null;
}
...
function documentIdentity(doc,index){
  return String(doc?.id || doc?.url || doc?.webViewLink || doc?.name || doc?.displayName || index);
}

function seenKey(token){
  return `lp28.organizer.documents.seen.${token}`;
}

function readSeen(token){
  try{
    const raw=JSON.parse(localStorage.getItem(seenKey(token))||"[]");
    return new Set(Array.isArray(raw)?raw:[]);
  }catch{return new Set();}
}

function writeSeen(token,ids){
  try{localStorage.setItem(seenKey(token),JSON.stringify([...ids]));}catch{}
}
...
function addBadge(nav,total,unread){
  ...
  let count=nav.querySelector(".lp28-doc-count-badge");
  if(!count){
    count=document.createElement("span");
    count.className="lp28-doc-count-badge";
    count.setAttribute("aria-label","Nombre de documents disponibles");
    nav.appendChild(count);
  }
  count.textContent=String(total);

  let fresh=nav.querySelector(".lp28-doc-new-badge");
  if(unread>0){
    if(!fresh){
      fresh=document.createElement("span");
      fresh.className="lp28-doc-new-badge";
      fresh.setAttribute("aria-label","Nouveaux documents");
      nav.appendChild(fresh);
    }
    fresh.textContent=`+${unread}`;
  }else if(fresh){
    fresh.remove();
  }
}
...
function paymentLabelFor(doc,data){
  ...
  if(type==="DEPOSIT_INVOICE" || name.includes("acompte")){
    if(payments.depositPaid) return {text:"🟢 Acompte reçu",cls:"is-paid"};
    return {text:"🟡 Acompte à venir",cls:""};
  }
...
function decorateDocumentLinks(data){
  const docs=visibleClientDocuments(data);
  if(!docs.length) return;
  const links=[...document.querySelectorAll("a.portal-action,a[href]")];

  docs.forEach(doc=>{
    const label=normalizeText(doc?.displayName || doc?.name || "");
    const href=doc?.url || doc?.webViewLink || "";
    const link=links.find(a=>(href && a.href===href) || (label && normalizeText(a.textContent).includes(label)));
    if(!link || link.dataset.lp28PaymentDecorated==="1") return;

    const status=paymentLabelFor(doc,data);
    const badge=document.createElement("span");
    badge.className=`lp28-payment-status ${status.cls}`.trim();
    badge.textContent=status.text;
    link.insertAdjacentElement("afterend",badge);
    link.dataset.lp28PaymentDecorated="1";
  });
}

async function loadPortalData(token){
  try{
    const r=await fetch(`/api/guest/${encodeURIComponent(token)}/portal`,{credentials:"include"});
    if(!r.ok) return null;
    return await r.json();
  }catch{return null;}
}
...
export function initOrganizerDocumentEnhancer(){
  ...
  const refreshDom=()=>{
    if(!data) return;
    const nav=findDocumentsNav();
    addBadge(nav,ids.length,unread);
    decorateDocumentLinks(data);

    if(nav && !navBound){
      navBound=true;
      nav.addEventListener("click",()=>{
        writeSeen(token,new Set(ids));
        unread=0;
  ...
  loadPortalData(token).then(portal=>{
    ...
    data=portal;
    const docs=visibleClientDocuments(portal);
    ids=docs.map(documentIdentity);

    const hasStored=localStorage.getItem(seenKey(token))!==null;
    const seen=readSeen(token);

    // Première activation de la fonction : on considère les documents déjà présents comme lus.
    // Les ajouts suivants déclencheront la notification lumineuse silencieuse.
    if(!hasStored){
      writeSeen(token,new Set(ids));
      unread=0;
    }else{
      unread=ids.filter(id=>!seen.has(id)).length;
    }
    refreshDom();
  });

  window.addEventListener("focus",async()=>{
    const portal=await loadPortalData(token);
    if(!portal || portal.role!=="ORGANIZER") return;
    data=portal;
    const docs=visibleClientDocuments(portal);
    ids=docs.map(documentIdentity);
