...
function MathisAssistant({videos=[],eventContext=null,userRole="admin",supportPhone="",portalToken=""}){
  ...
  const [printerStage,setPrinterStage]=useState("led-first");
  ...
  function IncidentReport(){
    ...
    return <div className="mathis-report">
      <div className="mathis-bubble mathis-bubble-bot"><b>📋 Compte rendu de votre technicien Mathis</b><br/>
        ...
        <b>État :</b> {printerAnswer==="solved"?"✅ incident résolu — notification silencieuse":printerAnswer==="failed"?"🔴 intervention niveau 2 demandée":"🟠 diagnostic en cours"}<br/>
  ...
  function PrinterDiagnostic(){
    if(printerStage==="led-first") return <>
      ...
      <div className="mathis-actions mathis-led-first-actions">
        ...
        <button onClick={()=>{setLedCode("");setPrinterStage("symptom")}}>🟢 NON, aucun voyant ne clignote</button>
      </div>
    </>;
...
function MathisPhotoAnalysis({incident}){
  const [busy,setBusy]=useState(false),[photo,setPhoto]=useState(null),[consent,setConsent]=useState(false),[copied,setCopied]=useState(false);
  const booth=String(incident.booth||"Borne");
  const trigger=/gabin/i.test(booth)?"Godox AT-16":"Godox X2T";
  const prompt=`Analyse cette photo dans le cadre d’un diagnostic technique pour une borne photobooth Location Photobooth 28.\n\nBorne : ${booth}\nAppareil : Nikon D7200\nDéclencheur flash : ${trigger}\nPuissance flash de référence : 1/8\nProblème signalé : ${incident.issue||"Qualité photo"}\nDiagnostic déjà recueilli : ${incident.diagnostic||incident.led||"Non précisé"}\n\nAnalyse uniquement ce qui est réellement visible sur la photo. Vérifie notamment l’exposition, la netteté, la dominante de couleur, le cadrage et si l’éclairage semble cohérent avec un déclenchement du flash.\n\nNe déduis pas qu’un flash est connecté uniquement à partir de l’image. Tu peux seulement indiquer s’il semble probablement avoir déclenché.\n\nDonne-moi ensuite :\n1. ton diagnostic probable ;\n2. les causes possibles ;\n3. ce que je dois contrôler sur la borne ;\n4. les réglages Nikon D7200 à vérifier : ISO, vitesse, ouverture et focale ;\n5. le réglage du flash à vérifier.\n\nJe joins la photo à ce message.`;
  async function loadPhoto(){setBusy(true);try{const r=await fetch(`/api/admin/mathis/incidents/${incident.id}/latest-photo`);const d=await r.json().catch(()=>({}));if(!r.ok)throw new Error(d.message||"Photo introuvable.");setPhoto(d.photo)}catch(err){alert(err.message||"Photo introuvable.")}finally{setBusy(false)}}
  async function copyPrompt(){try{await navigator.clipboard.writeText(prompt);setCopied(true);setTimeout(()=>setCopied(false),1800)}catch{const t=document.createElement("textarea");t.value=prompt;document.body.appendChild(t);t.select();document.execCommand("copy");t.remove();setCopied(true);setTimeout(()=>setCopied(false),1800)}}
  return <div className="mathis-photo-analysis"><div className="mathis-ai-consent-warning"><b>⚠️ Photo destinée à un diagnostic externe</b><p>LP28 n’envoie plus automatiquement cette photo à une IA. Si vous choisissez de la joindre vous-même à ChatGPT, vérifiez que toute personne visible a donné son accord.</p><label><input type="checkbox" checked={consent} onChange={e=>setConsent(e.target.checked)}/> Je confirme avoir l’accord nécessaire avant de récupérer et transmettre cette photo.</label></div><div className="mathis-manual-ai-actions"><button onClick={loadPhoto} disabled={busy||!consent}>📸 {busy?"Recherche…":"Récupérer la dernière photo"}</button><button onClick={copyPrompt}>🤖 {copied?"Message copié !":"Générer / copier le message ChatGPT"}</button></div>{photo&&<div className="mathis-manual-photo"><b>📷 Photo prête — {photo.source==="GALERIE"?"Galerie client":"Contrôle SAV"}</b><small>{photo.photoName||"Dernière photo"}{photo.photoTime?` · ${new Date(photo.photoTime).toLocaleString("fr-FR")}`:""}</small><img src={photo.viewUrl} alt="Dernière photo pour diagnostic"/><div className="mathis-manual-ai-actions"><a className="btn" href={photo.viewUrl} target="_blank" rel="noreferrer">🔎 Ouvrir la photo</a><a className="btn" href={photo.downloadUrl}>⬇️ Télécharger la photo</a><button onClick={copyPrompt}>📋 {copied?"Copié !":"Copier le message ChatGPT"}</button></div><details className="mathis-chatgpt-prompt"><summary>Voir le message généré</summary><pre>{prompt}</pre></details></div>}</div>
}
...
function Dashboard({onLogout,user}) {
  ...
  return <><LP28ThemeStyles/><div className={`app-shell ${mobileMenuOpen?"mobile-nav-open":""} role-${String(user?.role||"viewer").toLowerCase()}`}>
    ...
    <aside className={`sidebar ${mobileMenuOpen?"mobile-open":""}`}>
      <div className="brand"><img src="/logo-hd.png"/><div><strong>LP28 Suite</strong><span>Version 8.5.82</span></div></div>
      <nav>
        {navModules.filter(m=>{
