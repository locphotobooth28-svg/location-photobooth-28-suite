  ...
  function wrappedExpress(...args){
    ...
    app.post("/api/r2/portal-zip/:token",original.json({limit:"256kb"}),async(req,res)=>{
      try{
        ...
        const access=await prisma.portalAccess.findFirst({where:{token},include:{event:true}}).catch(()=>null);
        if(!access||access.role!=="ORGANIZER"||!access.event)return res.status(403).json({ok:false,message:"Accès organisateur requis."});
        ...
        const media=await prisma.memoryMedia.findMany({where:{id:{in:ids},eventId:access.event.id,deletedAt:null}});
        ...
        const zip=await buildZip(ordered,async item=>{
          ...
          const local=item.storagePath||(item.fileName?path.join(__dirname,"..","uploads","memories",item.fileName):null);
        ...
        const eventName=String(access.event.name||"evenement").normalize("NFKD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-zA-Z0-9_-]+/g,"-").replace(/^-+|-+$/g,"").slice(0,60)||"evenement";
        ...
        console.error("LP28 ZIP route :",err);
        if(!res.headersSent)res.status(500).json({ok:false,message:"Création du ZIP impossible."});
      }
    });
    return app;
  }
  Object.assign(wrappedExpress,original);
  wrappedExpress.__lp28ZipWrapped=true;
